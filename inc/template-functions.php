 ...
 * @return array
 */
function woolymar_body_classes( $classes ) {
...
add_filter( 'body_class', 'woolymar_body_classes' );

/**
 * Add a pingback url auto-discovery header for singularly identifiable articles.
 */
function woolymar_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'woolymar_pingback_header' );
 ...
 * @return array
 */
function woolymar_filter_script_loader_tag( $tag, $handle ) {
...
add_filter( 'script_loader_tag', 'woolymar_filter_script_loader_tag', 10, 2 );
 ...
 */
function woolymar_get_preload_stylesheet_uri( $wp_styles, $handle ) {
 ...
 */
function woolymar_add_body_style() {
	...
	// Preload content.css.
	$preloads['woolymar-content'] = woolymar_get_preload_stylesheet_uri( $wp_styles, 'woolymar-content' );

	// Preload sidebar.css and widget.css.
	if ( is_active_sidebar( 'sidebar-1' ) ) {
		$preloads['woolymar-sidebar'] = woolymar_get_preload_stylesheet_uri( $wp_styles, 'woolymar-sidebar' );
		$preloads['woolymar-widgets'] = woolymar_get_preload_stylesheet_uri( $wp_styles, 'woolymar-widgets' );
	}

	// Preload comments.css.
	if ( ! post_password_required() && is_singular() && ( comments_open() || get_comments_number() ) ) {
		$preloads['woolymar-comments'] = woolymar_get_preload_stylesheet_uri( $wp_styles, 'woolymar-comments' );
	}

	// Preload front-page.css.
	global $template;
	if ( 'front-page.php' === basename( $template ) ) {
		$preloads['woolymar-front-page'] = woolymar_get_preload_stylesheet_uri( $wp_styles, 'woolymar-front-page' );
	}
...
add_action( 'wp_head', 'woolymar_add_body_style' );
 ...
 */
function woolymar_add_primary_menu_dropdown_symbol( $item_output, $item, $depth, $args ) {
...
add_filter( 'walker_nav_menu_start_el', 'woolymar_add_primary_menu_dropdown_symbol', 10, 4 );
 ...
 */
function woolymar_add_nav_menu_aria_current( $atts, $item ) {
	/*
...
add_filter( 'nav_menu_link_attributes', 'woolymar_add_nav_menu_aria_current', 10, 2 );
add_filter( 'page_menu_link_attributes', 'woolymar_add_nav_menu_aria_current', 10, 2 );
